		...
		if($('#mobile_detect').val()==1 && i==-1 && showbanner==1){
			if(imgBannerUnit.length>0){
				for(b=0;b<m;b++){
					...
					imgNode += '<div class="carousel-caption-lb" style="margin-bottom:43px;width:100%;background: linear-gradient(rgba(255,255,255,0),rgba(51,51,51,0.8))">';
					...
					imgNode += '</div>';
					imgNode += '<div class="col-md-12  bg-white highlight-prakard text-center"><span style="color:#f36b22; font-size:18px;"><b>ขายด่วน! ลดพิเศษ 500,000บาท เจ้าของไปนอก!</b></span></div></div>';
		...
		for(var i=0;i<imgBanner.length;i++){
			...
			if(imgBanner[i].POID!='0'){
				...
				if(imgBanner[i].Link!=''){
					listBanner += '</a>';
					listBanner += '</div>';
					listBanner += '<div class="bg-white highlight-prakard text-center"><span style="color:#f36b22; font-size:18px;"><b>ขายด่วน! ลดพิเศษ 500,000บาท เจ้าของไปนอก!</b></span></div>';
	...
	if($('#mobile_detect').val()==1){
		if(imgBannerUnit.length>0){
			...
			for(b=0;b<imgBannerUnit.length;b++){
				...
				imgNode += '<div class="item active'+list_class+'" style="border:8px solid #f36b22;overflow:hidden;margin: 0px 0px;background-color:#ffffff; background-image:url('+imgBannerUnit[b].Picture+') ; background-repeat: no-repeat;background-position: center center;background-size:100%; padding-left: 0; padding-right: 0;position: relative; '+list_style+'">';
				if(imgBannerUnit[b].POID!='0'){
					imgNode += '<div class="carousel-caption-rt">';
					imgNode += '<div class="infogp-box">';
					imgNode += '<span class="infogp-data">'+imgBannerUnit[b].DateShow+'</span>';
