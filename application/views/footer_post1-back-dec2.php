...
function submitFormPage1()
{
	...
	if (f!="1")
	{
		document.getElementById("showAgree_Owner").className = "error_msg";
	} else {
		document.getElementById("showAgree_Owner").className = "normal_msg";
	}
	if (g=="")
	{
...
<?php
$endLine=$qProjectName->num_rows(); 
$i=1;
foreach ($qProjectName->result() as $row)
{
	if ($i!=$endLine){
		echo '"'.$row->PName_th.'",';
	} else {
