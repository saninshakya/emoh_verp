<?php
		$Advertising="";
		foreach ($qFavUnit->result() as $row){
		...
		?>
			...
			<div id="FavRow<?=$row->POID;?>" class="row col-md-12 col-sm-6 col-xs-12 padding-none3 sm-padding">
                ...
                <div class="col-md-4 text-shadow2 bg-responsive4" style="position: relative;background-image: linear-gradient(rgba(255,255,255,0),rgba(51,51,51,0.5)), url('<?=$this->dashboard->imageList($row->POID);?>');">
                  <div class="layer-invisible" onclick="window.open('/zhome/unitDetailOwner/<?=$row->POID;?>', '_blank')"><!--invisble layer-->
                  </div><!--end invisble layer-->
					...
					<div class="hidden-sm hidden-xs " style="position:absolute;top:2;right:0; padding-right:0px;">
						<span class="text-white pull-right hidden-sm hidden-xs">
							<a href="#" onclick="updateFavorite('<?=$row->POID;?>');" title="คลิกเพื่อเอาออกจากรายการที่สนใจ"><span width="3px" id="favourite_show" class="glyphicon glyphicon-heart text-pink hidden-sm hidden-xs" style="padding-top:3px;padding-right:5px;" aria-hidden="true" onmouseover="$(this).removeClass('text-pink');$(this).addClass('text-white');" onmouseout="$(this).removeClass('text-white');$(this).addClass('text-pink');">
							</span></a>
						</span>
					</div>
					<!--end fav bt-->
					...
					<div class="infodashboard visible-xs visible-sm">
						<div class="infodashboard-box pull-left" style="position:absolute;bottom:2;left:0;" onclick="window.open('/zhome/unitDetailOwner/<?=$row->POID;?>', '_blank')">
							...
							<div class="infodashboard-data-sub text-white padding-l20"><?=$ProjectName?> <span>(<?=$DistName?>)</span> </div>
						</div>
						...
						<div class="pull-right w60" style="position:absolute;top:2;right:0; margin-right: 0px;padding-right:10px;font-size:18px;">
							<div class="text-white text-right">
								...
								<span class="text-white">
							  		<a href="#" onclick="updateFavorite('<?=$row->POID;?>');">
										<span id="favourite_show_mb" class="infodashboard-fav glyphicon glyphicon-heart text-pink" aria-hidden="true"></span>
							  		</a>
								</span>
							</div>
				        </div>
				...
				<div class="visible-xs visible-sm col-xs-12 clear-margin-padding">
					<div class="col-xs-6 clear-margin-padding-mobile visible-xs visible-sm">
					...
					<div class="col-xs-6 clear-margin-padding-mobile visible-xs visible-sm">
	           	    ...
	           	    <div class="col-xs-6 clear-margin-padding-mobile visible-xs visible-sm">
						<button type="button" class="btn btn-green3" disabled="disabled"  data-toggle="modal" data-target="#myModal2" onclick="$('#modal_poid').val('<?=$row->POID;?>');">ติดตาม</button>
					</div>
					<div class="col-xs-6 clear-margin-padding-mobile visible-xs visible-sm ">
				...
				<div id="dash-list2" class="col-md-4 col-xs-6 padding-lr0 hidden-xs hidden-sm">
					...
					<div class="col-md-6" style="padding: 0px 6px 0px 0px">
						<button type="button" class="btn btn-green2 btn-dash padding-left-clear padding-right-clear text-center" disabled="disabled" data-toggle="modal" data-target="#myModal2" onclick="$('#modal_poid').val('<?=$row->POID;?>');">ติดตาม</button>
					</div>
				...
				<div class="modal-content text-center">
					...
					<div class="modal-body text-grey row text-left">
						...
						<div class="col-md-12 padding-top10">
							<select id="ptype" name="problem_type" class="form-control no-radius text-grey3 modal-warnning-select-item">
