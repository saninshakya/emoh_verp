			    ...
			    <div class="col-xs-6 padding-none text-center" style="padding-bottom: 0; padding-top: 0; margin-top:1px;">
                    <span class="pull-left w50">
						<button type="button" class="btn bg-white col-xs-12 border-left showfilter-bt">
							<img src="/img/red-point.png" class="checkfilter display-none" style="width:7px;height:7px; margin-bottom:3px;">
...
<!-- 					<div id="float_menu5" class="text-center hidden-xs hidden-sm">
			...
			<div class="w30x col-md-4-result padding-none clear-padding-lr padding-t10x" id="searchmapsidebar">
				...
				<div class="hidden-xs" id="searchmapshowcase" style="position: fixed; left: 70%;z-index: 100;width: 30%;">
					...
					<div id="paddingBanner" class="bg-grey3 display-none" style="height:57px;width:100%;"></div>
					<!--end banner-->
		...
		<div id="cImgUnitsMobile2" class="col-md-12 col-xs-12 padding-none q-sooth hidden-sm list-banner-mobile"></div><!--list banner on mobile-->
...
$(document).ready(function() {
	...
	$("#hidefilter").click(function(){
		...
		alert($('#sNBdroom_default').val());
		alert($('#sNBdroom_ref').val());
	...
	//if(imgUnits[0].Type!='Project'){
		if(mode==1){
			imgUnits.sort(function(a, b){
				var aPrice= parseFloat(a.Price), bPrice= parseFloat(b.Price);
				if(order==0){
				...
				return (a1 > b1)? 1 : (a1 < b1)? -1 :(aPrice > bPrice)? 1 : (aPrice < bPrice)? -1 : 0;
			});
		}
	//}
	if(pref==1){
		...
		if($('#mobile_detect').val()==1 && i==-1 && showbanner==1){
			if(imgBannerUnit.length>0){
				...
				for(b=0;b<m;b++){
					...
					imgNode += '<div style="border:8px solid #f36b22;" class="item active "><div class="'+list_class+' infosearchmap" style=";overflow:hidden;margin: 0px 0px;background-color:#ffffff; background-image:url('+imgBannerUnit[b].Picture+') ; background-repeat: no-repeat;background-position: center center;background-size:100%; padding-left: 0; padding-right: 0; '+list_style+'">';
					if(imgBannerUnit[b].POID!='0'){
						imgNode += '<div class="carousel-caption-rt">';
						imgNode += '<div class="infogp-box" style="padding-top:0px; padding-right:10px;">';
					...
					if(imgBannerUnit[b].Link!=''){
						if(imgBannerUnit[b].BannerStatus==1){
							banner_link='/bn';
						}
						imgNode += '<a href="/'+imgBannerUnit[b].NamePath+'/condo/'+imgBannerUnit[b].PName_en+'/'+imgBannerUnit[b].POID+''+banner_link+'" '+openLink+' alt="'+imgBannerUnit[b].AdvertisingName+'-'+imgBannerUnit[b].PropertyName+'-'+imgBannerUnit[b].ProjectNameAnchor+'" style="text-decoration: none;" onclick="updateViewBanner('+imgBannerUnit[b].POID+');">';
					}
					imgNode += '<div class="layer-invisible"></div>';
					imgNode += '<div class="carousel-caption-lb" style="margin-bottom:40px;width:100%;background: linear-gradient(rgba(255,255,255,0),rgba(51,51,51,0.8))">';
					...
					imgNode += '</div>';
					if(imgBannerUnit[b].AdTXT!=''){
						imgNode += '<div class="col-md-12  bg-white highlight-prakard text-center"><div class="highlight-prakard-text">'+imgBannerUnit[b].AdTXT+'</div></div>';
					}
					imgNode += '</div>';
	...
	if(imgBanner.length>0){
		...
		for(var i=0;i<imgBanner.length;i++){
			...
			if(imgBanner[i].POID!='0'){
				...
				listBanner += '<div style="border:8px solid #f36b22;" class="item '+active+'"><div class=" heightratio  infosearchmap" style="overflow:hidden;margin: 0px 0px;background-color:#ffffff; background-image:url('+imgBanner[i].Picture+') ; background-repeat: no-repeat;background-position: center center;background-size:100%; padding-left: 0; padding-right: 0;">';
				...
				listBanner += '</div>';
				if(imgBanner[i].Link!=''){
					if(imgBanner[i].BannerStatus==1){
						banner_link='/bn';
					}
					listBanner += '<a href="/'+imgBanner[i].NamePath+'/condo/'+imgBanner[i].PName_en+'/'+imgBanner[i].POID+''+banner_link+'" target="_blank" alt="'+imgBanner[i].AdvertisingName+'-'+imgBanner[i].PropertyName+'-'+imgBanner[i].ProjectNameAnchor+'" style="text-decoration: none;" onclick="updateViewBanner('+imgBanner[i].POID+');">';
				}

				listBanner += '<div class="layer-invisible"></div>';
				listBanner += '<div class="carousel-caption-lb" style="margin-bottom:41px;width:100%;background: linear-gradient(rgba(255,255,255,0),rgba(51,51,51,0.8))">';
				...
				if(imgBanner[i].Link!=''){
					listBanner += '</a>';
					listBanner += '</div>';
					if(imgBanner[i].AdTXT!=''){
						listBanner += '<div class="bg-white highlight-prakard text-center"><div class="highlight-prakard-text">'+imgBanner[i].AdTXT+'</div></div>';
					}
				}
				listBanner += '</div>';
				$('#paddingBanner').removeClass('display-none');
			}else{
				...
				listBanner += '</div>';
				$('#paddingBanner').addClass('display-none');
			}
		...
		if(imgBannerUnit.length>0){
			...
			for(b=0;b<imgBannerUnit.length;b++){
				...
				if(imgBannerUnit[b].Link!=''){
					...
					imgNode += '<a href="/'+imgBannerUnit[b].NamePath+'/condo/'+imgBannerUnit[b].PName_en+'/'+imgBannerUnit[b].POID+''+banner_link+'" target="_blank" alt="'+imgBannerUnit[b].AdvertisingName+'-'+imgBannerUnit[b].PropertyName+'-'+imgBannerUnit[b].ProjectNameAnchor+'" style="text-decoration: none;" onclick="updateViewBanner('+imgBannerUnit[b].POID+');">';
				}
				imgNode += '<div class="layer-invisible"></div>';
				imgNode += '<div class="carousel-caption-lb" style="width:100%;background: linear-gradient(rgba(255,255,255,0),rgba(51,51,51,0.8))">';
