...
function displayImgUnits(){
	...
	for(var i=0;i<imgUnits.length;i++){
        ...
        imgNode +=  '<div class="padding-l7"><h3 class="text-grey"> '+imgUnits[i].ProjectName+'</h3></div>';
        imgNode +=  '<div class="bg-grey2" style="position:relative; width:100%; background-color:#666666;height:257px;"><img src="'+imgUnits[i].Picture[0]+'" class="t-img-responsive"><h4  class="text-center" style="color:#ffffff; position:absolute; width:160px; bottom:0;right:0; margin-bottom:0; padding-top:8px; background-color: #2190ab; opacity:0.6">ZmyHome เท่านั้น</h4></div>';//width="360px" height="172px"
        imgNode +=	 '<table class="table borderless fsize-unitsearch">';
        imgNode +=	 '<tr>';
        imgNode +=	 '<td><strong><span class="text-grey">'+imgUnits[i].Bedroom+' นอน '+imgUnits[i].useArea+'ม&sup2;. ชั้น '+imgUnits[i].Floor+'</span></strong</td>';
        imgNode +=  '<td><strong>B <span class="text-primary">'+imgUnits[i].Price+'</span></span></td>';
        imgNode +=  '<td><strong>B <span class="text-primary text-right">'+imgUnits[i].PricePerSq+'</span></span>/ม&sup2;.</td>';
        imgNode +=  '</tr>';
        imgNode +=  '<tr>';
        imgNode +=  '<td>เจ้าของขายเอง</td>';
        ...
        imgNode +=  '<td><span class="text-blue text-right">'+imgUnits[i].Tel+'</span></td>';
        imgNode +=	 '</tr>';
        imgNode +=	 '</table>';
        imgNode +=	 '</div>';
        //imgNode +=   '</div>';     
