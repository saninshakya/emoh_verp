...
*/
  ...
  function duplicateRoom($PID,$Address,$RoomNumber,$Advertising=''){
	...
	function dailyAdSummary($start_date,$end_date){
		$query=$this->db->query('select COALESCE(operate_date,0) as sdate, COALESCE(operate_date,0) as operate_date, COALESCE(sum(credit_use), 0) as reach_promote_action,(select count(ID) from LogViewBanner where date(LastUpdate)=sdate) as click_promote_action,(select count(ID) from LogViewTel where type = 2 AND date(LastUpdate)=sdate) as tel_promote_action,(select count(Distinct ViewByUserID) from LogViewBanner where date(LastUpdate)=sdate) as click_promote_user,(select count(Distinct ViewByUserID) from LogViewTel where type = 2 AND date(LastUpdate)=sdate) as tel_promote_user from cCreditJobDetail where operate_date between "'.$start_date.'" and "'.$end_date.'"');
		$row=$query->row();
		$value[1]=$row->reach_promote_action;
		$value[2]=$row->click_promote_action;
		$value[3]=$row->tel_promote_action;
		//$value[4]=$row->reach_promote_user;
		$value[5]=$row->click_promote_user;
		$value[6]=$row->tel_promote_user;

		$query2=$this->db->query('select COALESCE(operate_date,0) as sdate, COALESCE(operate_date,0) as operate_date, COALESCE(sum(credit_use), 0) as reach_organic_action,(select count(ID) from LogViewBanner where date(LastUpdate)=sdate) as click_organic_action,(select count(ID) from LogViewTel where type = 1 AND date(LastUpdate)=sdate) as tel_organic_action,(select count(Distinct ViewByUserID) from LogViewBanner where date(LastUpdate)=sdate) as click_organic_user,(select count(Distinct ViewByUserID) from LogViewTel where type = 1 AND date(LastUpdate)=sdate) as tel_organic_user from cCreditJobDetail where operate_date between "'.$start_date.'" and "'.$end_date.'"');
		$row2=$query2->row();

		$value[7]=$row2->reach_organic_action;
		$value[8]=$row2->click_organic_action;
		$value[9]=$row2->tel_organic_action;
		$value[10]=$row2->click_organic_user;
		$value[11]=$row2->tel_organic_user;
		return $value;
	}
	
	function dailyAdSummaryDetail($start_date,$end_date){
		$query=$this->db->query('select cCreditJob.POID,cCreditJobDetail.operate_date,OwnerName,ProjectName, Bedroom, useArea, TotalPrice, PricePerSq from cCreditJob,cCreditJobDetail,Post where cCreditJob.JobID=cCreditJobDetail.JobID and cCreditJob.POID=Post.POID and cCreditJobDetail.operate_date between "'.$start_date.'" and "'.$end_date.'" group by cCreditJob.POID');
		return $query;
	}
	
	function dailyAdSummaryDetailUnit($operate_date,$POID){
		// $query=$this->db->query('select cCreditJob.POID as sPOID, operate_date as sdate, operate_date, sum(credit_use) as reach_promote_action, (SELECT count(ID) FROM LogViewBanner WHERE POID = sPOID AND date(LastUpdate)= sdate)AS click_promote_action, (SELECT count(ID) FROM LogViewTel WHERE POID = sPOID AND type = 2 AND date(LastUpdate)= sdate)AS tel_promote_action,(SELECT count(ID) FROM LogViewBanner WHERE POID = sPOID AND date(LastUpdate)= sdate) AS click_promote_user, (SELECT count(ID) FROM LogViewTel WHERE POID = sPOID AND type = 2 AND date(LastUpdate)= sdate) AS tel_promote_user FROM cCreditJob, cCreditJobDetail,Post WHERE cCreditJob.JobID = cCreditJobDetail.JobID AND cCreditJob.POID = Post.POID AND cCreditJobDetail.operate_date = "'.$operate_date.'" AND cCreditJob.POID = "'.$POID.'"');
		...
		$query2=$this->db->query('select cCreditJob.POID as sPOID, operate_date as sdate, operate_date, (SELECT count(ID) FROM LogViewPostDaily WHERE POID = sPOID AND date(LastUpdate)= sdate) AS click_organic_action, (SELECT count(ID) FROM LogViewTel WHERE POID = sPOID AND type = 1 AND date(LastUpdate)= sdate) AS tel_organic_action,(SELECT count(Distinct ViewByUserID) FROM LogViewPostDaily WHERE POID = sPOID AND date(LastUpdate)= sdate) AS click_organic_user, (SELECT count(Distinct ViewByUserID) FROM LogViewTel WHERE POID = sPOID AND type = 1 AND date(LastUpdate)= sdate) AS tel_organic_user FROM cCreditJob, cCreditJobDetail,Post WHERE cCreditJob.JobID = cCreditJobDetail.JobID AND cCreditJob.POID = Post.POID AND cCreditJobDetail.operate_date = "'.$operate_date.'" ');
		$row2=$query2->row();
		//$value[7]=$row2->reach_organic_action;
		$value[8]=$row2->click_organic_action;
		$value[9]=$row2->tel_organic_action;
