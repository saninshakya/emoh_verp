...
class Zhome extends CI_Controller {
	...
	public function dashboard()
	{
		...
		$user_id=$this->session->userdata('user_id');
		$this->backend->addLogClick($user_id,'dashboard');
	...
	public function post_v2()
	{
		...
		$data['qTOAdvertising']=$this->post->qTOAdvertising();
		$data['qPost']=$this->post->qPost();
		$data['qLabel']=$this->search->qLabel('post',$language);
	...
	public function post5v2()
	{
		...
		$data['qLabel']=$this->search->qLabel('post5',$language);
		$this->load->view($header,$profile);
		$this->load->view('inputform5v2',$data);
		$this->load->view('footer_post5',$data);
	}
...
/// add by tas on 17 Aug 15 ///
	...
	public function post4v2(){
		...
		$data['qLabel']=$this->search->qLabel('post4',$language);
		$this->load->view($header,$profile);
		$this->load->view('inputform4v2',$data);
		$this->load->view('footer_post4',$data);
...
//			$tsegs = sizeof($segs);
		...
		if (!is_numeric($POID)){
			...
			$BannerStatus = $this->uri->segment(5);
		}
		if ($POID=="1"){
			$POID = $this->uri->segment(5);
			$BannerStatus = $this->uri->segment(6);
		}
		...
		if ($CheckBlocking==0){
			...
			if($BannerStatus==''){
				$this->unitdetail->recordViewPost($POID);
			}
			$this->search->updateViewPost($POID);
			$data['unit']=$this->search->getUnitFromPOID($POID);
			$data['project']=$this->unitdetail->getProjectFromPOID($POID);
			...
			$data['checkQuestionnair']=$this->unitdetail->checkQuestionnair($POID);
			if($firstname!=''){
...
//				$record=$outFile;
     ...
     public function renewpostbymessenger()
     {
     	$checkRobot=$this->usermenu->check_robot();
     	if ($checkRobot!=1){
     		$POID=$this->uri->segment(3);
     		$Token=$this->uri->segment(4);
     		$this->postemail->renewpostbymessenger($POID,$Token);
     		$Data['Msg']="ประกาศของคุณได้รับการต่ออายุอีก 30 วันเรียบร้อยแล้ว";
     		$this->load->view('header_nonauth');
     		$this->load->view('message',$Data);
     		$this->load->view('footerstandard');
     ...
     public function getImageBanner(){
     	...
     	$res=$this->search->getImageBanner($viewtype,$distance,$namePoint,$TOProperty,$Bedroom,$Bathroom,$TransDist,$TransType,$Year,$TOAdvertising,$minPrice,$maxPrice,$OrderType,$loopPOID);
     }
	...
	function boostPost(){
		...
		if($ChkLogin==1){
			...
			$user_id=$this->session->userdata('user_id');
			$this->credit->addlogbutton($user_id,$data['POID'],1);
			$checkstatus=$this->credit->CheckStatusBoostPost($data['POID']);
			if ($checkstatus==0 or $checkstatus==6){
	...
	function boostPostAlert(){
		...
		if($ChkLogin==1){
			...
			$profile=$this->user_profile($ChkLogin);
			$this->load->view($header,$profile);
			$this->load->view('boostpost_alert',$data);
			$this->load->view('dashboard_footer',$data);
		}
	}

	function boostPostView(){
		$this->session->set_userdata('lastpage','/');
		$this->lang_check();
		$ChkLogin=$this->ChkLogin();
		if($ChkLogin==1){
			...
			$data['POID']=$this->uri->segment(3);
			$data['date_type']=$this->uri->segment(4);
			$this->credit->addlogbutton($user_id,$data['POID'],3);
			$data['ListPostType1']=$this->credit->ListPost($data['POID']);
			$data['ListUnPost']=$this->dashboard->ListUnPost($user_id);
			//query data from old dashboard
			$data['unlist']=$this->dashboard->check_unlist();
			...
			$profile=$this->user_profile($ChkLogin);
			$this->load->view($header,$profile);
			$this->load->view('boostpost_view',$data);
			$this->load->view('dashboard_footer',$data);
		}
	}
	...
	public function submitBoostPost()
	{
      ...
      //Wait for change code
			...
			$end_date_array=explode("/",$_POST['defineDayBoostPost']);
			$end_date=date("Y-m-d H:i:s",mktime(date("H"), date("i"), date("s"), $end_date_array[1]  , $end_date_array[0], $end_date_array[2]));
		}
	...
	public function usePromotion(){
		$PromoCode=$_POST["PromoCode"];
		$result = $this->credit->usePromotion($PromoCode);
		echo $result;
	}
	...
	public function payment_success(){
		$key_load=$this->uri->segment(3);
		$PromoCode=$this->uri->segment(4);
		$PromoPoint=$this->uri->segment(5);
		if($key_load<>''){
			$data['key_load']=$key_load;
			$data['PromoCode']=$PromoCode;
			$data['PromoPoint']=$PromoPoint;
			$user_data=$this->users->get_user_by_id($this->tank_auth->get_user_id(),1);
			$data['firstname'] = $user_data->firstname;
			$data['lastname'] = $user_data->lastname;
			$data['username']=$user_data->firstname." ".$user_data->lastname;
			$data['email']=$this->postemail->get_email_from_userid($this->tank_auth->get_user_id());
			$ChkLogin=$this->ChkLogin();
			$header=$this->header_inc($ChkLogin);
			$profile=$this->user_profile($ChkLogin);
			$this->load->view($header,$profile);
	...
	public function cancelBoostPost(){
		$JobID=$_POST['JobID'];
		$POID=$_POST['POID'];
		$result = $this->credit->cancelBoostPost($JobID,$POID);
		echo $result;
	}

	function boostPostBlock(){
		$this->session->set_userdata('lastpage','/');
		$this->lang_check();
		$ChkLogin=$this->ChkLogin();
		if($ChkLogin==1){
			...
			$profile=$this->user_profile($ChkLogin);
			$this->load->view($header,$profile);
			$this->load->view('boostpost_blocknote',$data);
			$this->load->view('dashboard_footer',$data);
		}
	}
	public function ad(){
	...
	public function photographer(){
		...
		$profile=$this->user_profile($ChkLogin);
		$this->load->view($header,$profile);
		$this->load->view('photographer',$data);
		$this->load->view('footerstandard');
	}
	...
	public function updateViewBannerTel(){
		$POID=$_POST["POID"];
		$result = $this->search->updateViewBannerTel($POID);
		//echo $result;
	}

	public function uploadResizedBoostPost()
	{
		error_log('AAA');
		error_log($_POST['POID']);
		if ($_POST) {
			$userid=$this->session->userdata('user_id');
			$token=$this->session->userdata('token');
			define('IMG_DIR', 'userImg/'.$userid.'/');
			...
			// calculate the position
             ...
             $record=$outFile;
             $POID=$_POST['POID'];
             $this->db->query('update ImagePost set Order_Sort=Order_Sort+1 Where POID="'.$POID.'"');
             $this->db->query('insert into ImagePost set POID="'.$POID.'", file="/'.$record.'", type="room", Horizental="1", Order_Sort="1"');
         }
     }

     public function submitBoostPost2()
     {
     	$AdTXT=$_POST['textBoostPost'];
     	$POID=$_POST['POID'];
    //echo $POID."<br>";
    //echo $AdTXT."<br>";
     	$query=$this->db->query(
     		'Insert Into `cCreditJob` (
     		`POID`,`user_id`,`Package`,`credit_limit_pday`,`length_date`,`boost_date`,`start_date`,`end_date`,`end_job`,`Active`,`DateTime`,`AdTXT`)
     		Select `POID`,`user_id`,`Package`,`credit_limit_pday`,`length_date`,`boost_date`,`start_date`,`end_date`,`end_job`,"7",`DateTime`,"'.$AdTXT.'" from cCreditJob Where POID="'.$POID.'" and Active="5" Order by JobID DESC Limit 1');
     	$this->dashboard();

     }

     public function recordActionBoostPost(){
     	$JobID=$_POST["JobID"];
     	$ActionID=$_POST["ActionID"];
     	$result = $this->credit->recordActionBoostPost($JobID,$ActionID);
		//echo $result;
     }

     public function recordViewTel(){
     	$POID=$_POST["POID"];
     	$result = $this->unitdetail->recordViewTel($POID);
		//echo $result;
     }

     public function projectAdword(){
     	$ChkLogin=$this->ChkLogin();
     	$checkAdmin=$this->backend->checkAdmin();
     	if ($checkAdmin==1){
     		$data['listProject']=$this->backend->listProjectAdword();
     		$header=$this->header_inc($ChkLogin);
     		$profile=$this->user_profile($ChkLogin);
			//$this->load->view($header,$profile);
     		$this->load->view('listProjectAdword',$data);
			//$this->load->view('footer');
     	}
     }

     public function add_Questionnair(){
     	$this->unitdetail->add_Questionnair($_POST);
     }
 }
