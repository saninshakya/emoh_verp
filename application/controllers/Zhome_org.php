	...
	{	
		...
		if ($CheckBlocking==0){			
			...
			if($TOAdvertising==1){
				...
				$PriceShow=$Price;
				$PriceUnit=" บาท";
				$AdvertisingName="ให้".$AdvertisingName;
			}
			$title=$AdvertisingName.$PropertyName." ".$data['rowUnit']->ProjectName." เจ้าของขายเอง";
			if($data['rowUnit']->Bedroom=='99'){
				$BedroomShow="สตูดิโอ";
			}else{
				$BedroomShow=$data['rowUnit']->Bedroom." ห้องนอน";
			}
			//$title.="-".strlen($title);
			$description=$AdvertisingName.$PropertyName." ".$BedroomShow." ".$data['rowUnit']->Bathroom." ห้องน้ำ ชั้น ".$data['rowUnit']->Floor." ขนาด ".$data['rowUnit']->useArea." ตร.ม. ราคา ".number_format($PriceShow,1).$PriceUnit." หรือ ".number_format($data['rowUnit']->PricePerSq,0)." บาท/ตร.ม. อยู่ห่างรถไฟฟ้า ".$data['rowTrain']->KeyName." ".number_format($data['rowTrain']->Distance,0)." ม. สร้างเสร็จปี พ.ศ.".$data['rowProject']->YearEnd." ".$unitSpec.$projectAddress;
			$ChkLogin=$this->ChkLogin();
			if ($mfb=="mfb"){
				$header="header_mfb";
				$data2['title']=$title;
