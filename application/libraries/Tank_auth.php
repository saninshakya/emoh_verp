...
class Tank_auth
{
	 ...
	 * @return	bool
	 */
	function login_oa2($login, $image, $facebookID, $FileNameImage)
	{
		if ((strlen($login) > 0)) {
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login,$facebookID,$FileNameImage))) {	// login ok
	...
	function logout()
	{
		...
		$this->ci->session->set_userdata(array('user_id' => '', 'username' => '', 'messengerID' => '', 'status' => ''));
	 ...
	 * @return	array
	 */
	function create_user_oa2($email, $provider, $firstname, $lastname, $facebookID, $FileNameImage)
	{
